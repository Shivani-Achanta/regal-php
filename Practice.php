...
// Retrieve the length of the Questions_solved JSON attribute from tbl_users for the logged-in user
$sql = "SELECT JSON_LENGTH(Questions_solved) AS total_questions_solved FROM tbl_users WHERE id = ?";
...
if ($result->num_rows > 0) {
    ...
    if (!is_null($row['total_questions_solved'])) {
        $currentQuestionIndex = $row['total_questions_solved'];
    }
} else {
...
$sql = "SELECT q.*, t.topic, t.subtopic 
        ...
        LEFT JOIN tbl_users u ON JSON_CONTAINS(u.Questions_solved, JSON_QUOTE(q.id))
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        // Fetch the topic name for each question and add it to the $questions array
        $question_id = $row['id'];
        $topic_name = ""; // Initialize the topic name variable
        $topic_query = "SELECT t.topic 
                        FROM topics t
                        JOIN questions_list q ON q.subtopicid = t.id
                        WHERE q.id = ?";
        $topic_stmt = $conn->prepare($topic_query);
        $topic_stmt->bind_param("i", $question_id);
        $topic_stmt->execute();
        ...
        $row['topic'] = $topic_name; // Add the topic name to the current question row
        $questions[] = $row; // Add the modified row to the $questions array
    }
}
...
<body class="bg-blue-500 ">
  ...
  function enableButtons() {
    ...
    function updateReasoning(questionId, userId) {
      $.ajax({
        type: "POST",
        url: "Updates/update_reasoning.php",
        data: {
          question_id: questionId,
          user_id: userId
        },
    ...
    function updateGeneral(questionId, userId) {
      $.ajax({
        type: "POST",
        url: "Updates/update_general.php",
        data: {
          question_id: questionId,
          user_id: userId
        },
    ...
    function updateAptitude(questionId, userId) {
      $.ajax({
        type: "POST",
        url: "Updates/update_aptitude.php",
        data: {
          question_id: questionId,
          user_id: userId
        },
    ...
    function updateLogical(questionId, userId) {
      $.ajax({
        type: "POST",
        url: "Updates/update_logical.php",
        data: {
          question_id: questionId,
          user_id: userId
        },
    ...
    if (currentQuestionIndex >= 0 && currentQuestionIndex <= questions.length) {
        ...
        // Call different update functions based on the topic
        switch (topic) {
            case 'Reasoning':
                updateReasoning(questionId, userId);
                break;
            case 'English':
                updateEnglish(questionId, userId);
                break;
            case 'General Knowledge':
                updateGeneral(questionId, userId);
                break;
            case 'Aptitude':
                updateAptitude(questionId, userId);
                break;
            case 'Logical':
                updateLogical(questionId, userId);
                break;
            default:
                break;
        }
    ...
    function initializeQuiz() {
      ...
      document.getElementById('next-btn').addEventListener('click', function() {
        ...
        loadQuestion();
        // Disable buttons after loading new question
      document.getElementById('next-btn').disabled = true;
      document.getElementById('open-btn').disabled = true;
      });
    }
