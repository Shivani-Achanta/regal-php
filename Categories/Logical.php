...
// Retrieve the length of the Logical JSON attribute from tbl_users for the logged-in user
$sql = "SELECT JSON_LENGTH(Logical) AS total_logical FROM tbl_users WHERE id = ?";
...
if ($result->num_rows > 0) {
  ...
  if (!is_null($row['total_logical'])) {
    $currentQuestionIndex = $row['total_logical'];
  }
} else {
...
JOIN topics t ON q.subtopicid = t.id AND t.topic = 'Logical'
LEFT JOIN tbl_users u ON JSON_CONTAINS(u.Logical, JSON_QUOTE(q.id))
