...
// Retrieve the length of the General JSON attribute from tbl_users for the logged-in user
$sql = "SELECT JSON_LENGTH(General) AS total_general FROM tbl_users WHERE id = ?";
...
if ($result->num_rows > 0) {
  ...
  if (!is_null($row['total_general'])) {
    $currentQuestionIndex = $row['total_general'];
  }
} else {
...
JOIN topics t ON q.subtopicid = t.id AND t.topic = 'General Knowledge'
LEFT JOIN tbl_users u ON JSON_CONTAINS(u.General, JSON_QUOTE(q.id))
...
<head>
  ...
  <title>Regal | General Knowledge</title>
