...
// Retrieve the length of the Aptitude JSON attribute from tbl_users for the logged-in user
$sql = "SELECT JSON_LENGTH(Aptitude) AS total_aptitude FROM tbl_users WHERE id = ?";
...
if ($result->num_rows > 0) {
  ...
  if (!is_null($row['total_aptitude'])) {
    $currentQuestionIndex = $row['total_aptitude'];
  }
} else {
...
JOIN topics t ON q.subtopicid = t.id AND t.topic = 'Aptitude'
LEFT JOIN tbl_users u ON JSON_CONTAINS(u.Aptitude, JSON_QUOTE(q.id))
