...
// Retrieve the length of the Reasoning JSON attribute from tbl_users for the logged-in user
$sql = "SELECT JSON_LENGTH(Reasoning) AS total_reasoning FROM tbl_users WHERE id = ?";
...
if ($result->num_rows > 0) {
  ...
  if (!is_null($row['total_reasoning'])) {
    $currentQuestionIndex = $row['total_reasoning'];
  }
} else {
...
JOIN topics t ON q.subtopicid = t.id AND t.topic = 'Reasoning'
LEFT JOIN tbl_users u ON JSON_CONTAINS(u.Reasoning, JSON_QUOTE(q.id))
