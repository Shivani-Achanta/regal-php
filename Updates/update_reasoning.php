...
try {
    // Retrieve the current Reasoning JSON for the user
    $sql = "SELECT Reasoning FROM tbl_users WHERE id = ?";
    ...
    $questionsSolved = json_decode($row['Reasoning'], true);
    ...
    $sql = "UPDATE tbl_users SET Reasoning = ? WHERE id = ?";
    $stmt = $conn->prepare($sql);
    ...
    echo json_encode(['status' => 'success', 'message' => 'Reasoning updated successfully.']);
} catch (Exception $e) {
