...
try {
    // Retrieve the current General JSON for the user
    $sql = "SELECT General FROM tbl_users WHERE id = ?";
    ...
    $questionsSolved = json_decode($row['General'], true);
    ...
    $sql = "UPDATE tbl_users SET General = ? WHERE id = ?";
    $stmt = $conn->prepare($sql);
    ...
    echo json_encode(['status' => 'success', 'message' => 'General updated successfully.']);
} catch (Exception $e) {
