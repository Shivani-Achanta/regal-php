...
try {
    // Retrieve the current Aptitude JSON for the user
    $sql = "SELECT Aptitude FROM tbl_users WHERE id = ?";
    ...
    $questionsSolved = json_decode($row['Aptitude'], true);
    ...
    $sql = "UPDATE tbl_users SET Aptitude = ? WHERE id = ?";
    $stmt = $conn->prepare($sql);
    ...
    echo json_encode(['status' => 'success', 'message' => 'Aptitude updated successfully.']);
} catch (Exception $e) {
