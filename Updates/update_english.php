...
try {
    // Retrieve the current English JSON for the user
    $sql = "SELECT English FROM tbl_users WHERE id = ?";
    ...
    $questionsSolved = json_decode($row['English'], true);
    ...
    $sql = "UPDATE tbl_users SET English = ? WHERE id = ?";
    $stmt = $conn->prepare($sql);
    ...
    echo json_encode(['status' => 'success', 'message' => 'English updated successfully.']);
} catch (Exception $e) {
